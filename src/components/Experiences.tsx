        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {ITEMS.map((item) => (
            <div
              key={item.label}
              data-fade-up
              className="opacity-0 flex items-center gap-4 p-5 rounded-sm bg-white/[0.02] border border-white/[0.05] hover:border-support/20 hover:bg-white/[0.04] transition-all duration-400"
            >
              <div className="w-10 h-10 rounded-sm bg-support/10 flex items-center justify-center shrink-0">
                <item.icon className="w-5 h-5 text-support-light" />
              </div>
              <div>
                <p className="font-medium text-white text-base">{item.label}</p>
                <p className="text-base text-white/80">{item.sub}</p>
              </div>
            </div>
          ))}
        </div>
