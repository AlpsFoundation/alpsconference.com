    ...
    <section ref={sectionRef} id="location" className="relative py-24 sm:py-32">
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid lg:grid-cols-2 gap-10 items-center">
          {/* Map embed */}
          <div data-fade-up className="opacity-0 aspect-video rounded-2xl overflow-hidden border border-white/[0.06]">
            <iframe
          ...
          <div className="space-y-6">
            <p data-fade-up className="opacity-0 text-secondary/70 leading-relaxed">
            ...
            <div data-fade-up className="opacity-0 grid grid-cols-2 gap-3 pt-2">
              {FEATURES.map((f) => (
                <div key={f.text} className="flex items-start gap-2.5 text-sm">
                  <f.icon className="w-4 h-4 text-support mt-0.5 shrink-0" />
                  <span className="text-secondary/60">{f.text}</span>
                </div>
              ))}
            </div>
          </div>
