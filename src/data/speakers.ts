/** Shared speaker records used by the conference site and the print booklet. */
...
export type TbdSpeaker = {
  tbd: true;
};

export type SpeakerEntry = Speaker | TbdSpeaker;

export type FaceBox = readonly [left: number, top: number, width: number, height: number];

export type SpeakerImageCrop = {
  position: string;
  scale: number;
...
export const DEFAULT_IMAGE_CROP: SpeakerImageCrop = {
  position: "50% 38%",
  scale: 1,
  faceBox: [35, 20, 30, 30],
};
...
export const SPEAKER_IMAGE_CROPS: Record<string, SpeakerImageCrop> = {
...
export function getImageCrop(image?: string) {
  return image ? SPEAKER_IMAGE_CROPS[image] ?? DEFAULT_IMAGE_CROP : DEFAULT_IMAGE_CROP;
}

export function getFaceCenter([left, top, width, height]: FaceBox) {
  return `${left + width / 2}% ${top + height / 2}%`;
}

export function isConfirmedSpeaker(entry: SpeakerEntry): entry is Speaker {
  return !("tbd" in entry && entry.tbd);
}
...
export const SPEAKERS: SpeakerEntry[] = [
  {
    ...
    abstract:
      "Current regulatory frameworks struggle to accommodate the integration of pharmacological and psychotherapeutic elements in psychedelic therapy. Drug developers therefore portray psychotherapeutic interventions (“psychological support”) as a mere safety measure while attributing treatment efficacy to the drug alone. These efforts lack convincing empirical evidence and risk obscuring the mechanisms through which these treatments produce therapeutic change, thereby potentially undermining the development and delivery of effective clinical interventions.\n\nThis talk proposes that a more appropriate framework may be to evaluate psychedelics as psychotherapeutic tools, i.e. adjunctive drugs that can promote psychotherapeutic change processes when administered within a psychotherapeutic context. To support this view, I present evidence that outcome-predictive aspects of psychedelic experiences correspond to general change mechanisms of psychotherapy, including resource activation (mobilizing adaptive capacities), problem actuation (engaging emotionally with psychological problems), clarification (gaining self-understanding) and mastery (learning adaptive coping abilities). To illustrate the role of psychedelics as potent facilitators of these processes, I present results from the EPIsoDE trial (psilocybin for depression in N=144 patients) suggesting that acceptance-related learning experiences during dosing sessions mediate rapid, enduring reductions in experiential avoidance, an otherwise highly stable dysfunctional pattern that psychotherapy commonly seeks to attenuate. At the within-person level, post-dosing experiential avoidance predicted subsequent depression severity, but not vice versa, pointing to avoidance reduction as a psychedelic-enabled psychotherapeutic process that plausibly contributes to symptom improvement.\n\nAlthough empirical evidence supports the view of psychedelics as psychotherapeutic tools, this view may be assumed to undermine psychedelic regulation, based on the frequently invoked fact that drug regulators “do not regulate psychotherapy”. However, this concern confuses the regulation of an adjunctive drug with the regulation of the broader therapeutic intervention in which the drug is used. Drawing on an analogy with anesthetics – another class of adjunctive drugs that facilitate therapeutic interventions by inducing temporary changes in consciousness – I outline what a regulatory approach that evaluates psychedelics as adjuncts to psychotherapy might look like.",
  ...
  },
  {
    ...
    abstract:
      "Psychedelics can unsettle the assumptions through which a person holds reality together: what is real, what the self is, what matters. Whether such unsettling is considered mystical or challenging, desirable or adverse, it is often assumed that the experience itself determines the outcome. Drawing on five studies I argue for a third reading. Ontological destabilisation is pluripotent. The same disruption can lead to enduring distress or to substantive developmental gain, and which trajectory unfolds is shaped largely in the aftermath, interacting with the resources one carries in and can reach afterwards. I discuss findings on the generative trajectory of diversification, an enlarged repertoire of ways of holding reality, coupled with the agility to move between them and marked by a capacity to hold ambiguity and complexity. The talk introduces the Ontologically Diversifying Experiences (ODE) framework and three findings that follow from it: trust as an enabler of access to the resources integration requires; the integration gap in Western secular contexts, where people often lack both the language for what happened and the settings in which it could be metabolised; and the developmental paradox, whereby those whose trust and scaffolding are most compromised are both most likely to be destabilised and least able to reach support, a question of structural justice. I close with what this asks of care: an approach that stops treating ontological disruption as pathology by default and stresses the conditions of navigation as a collective responsibility rather than an individual achievement.",
    bio: "Dr Eirini K. Argyri is a researcher and educator working at the intersection of psychedelic studies, transformative learning, and integration support. Her work examines how experiences that challenge ordinary assumptions about self, reality, and meaning can lead to transformation and how associated distress can be better understood, supported, and mitigated when it emerges. Alongside her research, Eirini delivers training and offers consultancy on the phenomenology of challenging experiences and integration support, and volunteers with PsyCare UK, a peer-support welfare service at music festivals. She is committed to raising awareness of the complexity of psychedelic experiences and the plurality of their possible outcomes.",
  },
  {
