...
Use the model ID from the environment (e.g. `Codex-sonnet-4-6`). Estimate token counts from the response length if exact figures are unavailable. Use Anthropic's published per-token pricing to compute the cost estimate.
